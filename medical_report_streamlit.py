...
if selected_page == "Questões Análise":
    ...
    # Função de separação dos dados em treino e teste
    ...
    def pipeline_teste(df):
        pipeline = Pipeline([
            ...
            ('ordinal_feature', OrdinalEncoder()),
    ...
    teste_novo_cliente = pd.DataFrame(teste_novo_cliente)

# Agora você pode usar o drop sem problemas
    cliente_pred = teste_novo_cliente.drop(['DoencaVascular'], axis=1, errors='ignore')

    # Predições
    if st.button('Enviar'):
        model = joblib.load('modelo/xgb.vascular_1')
